...
Compression was tested in isolation (COMPx conditions, no IDK) and IDK's contribution was isolated by removal from the full OGS framework — dropping it sent one model's hallucination rate to 100%. The parsing prefix isn't a candidate for isolation testing at all: it has no effect on its own, only on whether the directive following it gets read as a constraint rather than a topic. Testing it without something to frame would test nothing. The two-part refusal instruction, likewise, was never split-tested against itself as separate conditions; that's a functional distinction drawn from how the prompt behaves, not a comparison run in the dataset. Full methodology, condition definitions, and trial-level data are in the papers and dataset linked below.
